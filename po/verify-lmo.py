...
Adapted from the same script in luci-app-filexplorer, which shares this
project's deploy-by-copying approach to translations.
...
What it checks, per message in the .po:
  ...
  * the key LuCI will look up is present in the catalog (the .lmo is
    read server-side here: the page is a Lua template, so <%: %> and
    luci.i18n.translate() hash the msgid with the very same
    SuperFastHash the C library uses)
...
Usage:  python3 po/verify-lmo.py po/ru/overlaybackup.po \\
...
def main():
    ...
    for msgid, plural, strs, ctxt in msgs:
        ...
        # and LuCI builds the very same string when it looks a message up.
